...
### @メンションによるファイル指定
ユーザーのメッセージに \`[添付ファイル: ファイル名 (パス: /Volumes/...)]\` という情報が含まれる場合、ユーザーはそのファイルについて質問しています。
**必ず server02_read_file アクションでそのファイルの内容を取得してから回答してください。** パスはそのまま file_path パラメータに指定してください。
